...
export async function researchTokenService(req: TokenResearchRequest & { query?: string; chartDays?: number }): Promise<StructuredAnalysisResult & { rawLegacyData?: Record<string, unknown> }> {
  ...
  let aiParsed = {
    ...
    strengths: ['Verified Smart Contract', 'Sufficient Market Liquidity'],
    risks: ['Market volatility'],
    riskScore: 25,
    riskLevel: 'Low Risk',
    riskReason: 'Established liquidity and verified contract',
    securityChecks: {
    ...
    liquidityHealth: {
      dex: 'OKX / DEX',
      liquidityAmount: '$1.5M',
      lpStatus: 'Locked',
      trading: 'Healthy',
    },
    verdict: {
      status: 'SAFE TO EXPLORE',
      message: 'Token fundamentals and smart contract checks pass basic safety requirements.',
    },
  };
