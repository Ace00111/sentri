...
export default function Settings() {
  ...
  const toggleStrictSecurity = () => {
    const newVal = !strictSecurity
    setStrictSecurity(newVal)
    localStorage.setItem('sentri_strict_security', String(newVal))
  }

  const cycleLanguage = () => {
    const langs = ['English (US)', 'Spanish', 'French', 'Japanese'] as const
  ...
  return (
    <div className="max-w-4xl">
      <h1 className="text-3xl font-bold mb-10">{t('settings')}</h1>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-darkBg/50 border border-white/5 rounded-2xl p-6 flex items-center justify-between" data-purpose="setting-wallet">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-semibold text-lg">{t('walletConnection')}</h3>
              <p className="text-mutedText text-sm">
                {isConnected ? `${address?.slice(0,6)}...${address?.slice(-4)} · ${t('walletConnected')}` : t('walletNotConnected')}
              </p>
            </div>
          </div>
          {isConnected ? (
        ...
        <div className="bg-darkBg/50 border border-white/5 rounded-2xl p-6 flex items-center justify-between" data-purpose="setting-security">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="font-semibold text-lg">{t('securityPreferences')}</h3>
              <p className="text-mutedText text-sm mt-1">{strictSecurity ? 'Strict Mode (High Sensitivity)' : 'Standard Mode'}</p>
            </div>
          </div>
          <button 
            onClick={toggleStrictSecurity}
            className={`w-14 h-8 rounded-full relative flex items-center px-1 transition-colors ${strictSecurity ? 'bg-red-500' : 'bg-zinc-700'}`}
          >
            <div className={`w-6 h-6 bg-black rounded-full shadow-md transition-transform ${strictSecurity ? 'ml-auto' : 'mr-auto'}`}></div>
          </button>
        </div>
      </div>
    </div>
